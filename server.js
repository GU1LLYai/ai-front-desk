...
import { provisionNumber, useExistingNumber } from "./lib/provision.js";
...
  const { businessName, countryCode, areaCode, existingNumber, openTime, closeTime, daysOpen, servicesText, faqText } =
    req.body;

  let phoneNumber;
  try {
    phoneNumber = existingNumber
      ? await useExistingNumber(existingNumber.trim())
      : await provisionNumber({ countryCode: countryCode || "GB", areaCode: areaCode || undefined });
  } catch (err) {
